/-
  ZeroWindingAssembly.lean — Thin routing to CaseObstructionsCore

  Provides `subThreshold_obstruction_v2` with the same signature as
  `subThreshold_obstruction` (CaseObstructions.lean line 1191).

  Architecture:
    ZeroWindingAssembly imports:
      - CycleTypes (cycle infrastructure)
      - Shadow.Theorem (shadow cycle mirror theorem)
      - GlobalMinGap (zero-winding large-arc machinery)
      - CaseObstructionsCore (4 case-split stubs)

  Case decomposition:
    1. Safe processor exists → small_arc_contradicts_convergence' (sorry-free)
    2. Zero winding, cw = 0 → all_stay_contradicts_convergence' (sorry-free)
    3. Zero winding, cw > 0 → zeroWinding_large_arc_false, routes to:
       A. consecutive binary → zeroWinding_consecutive_false (CaseObstructionsCore)
       B. non-consecutive binary → zeroWinding_nonConsecutive_false (CaseObstructionsCore)
    4. Non-zero winding → nonZeroWinding_false, routes to:
       C. sweep → sweep_false (CaseObstructionsCore)
       D. odd-winding non-uniform → oddWinding_nonUniform_false (CaseObstructionsCore)

  All sorrys live in CaseObstructionsCore.lean (the 4 stubs).
-/
...
/-! ### Helper: moverAt is constant when all steps are stay -/
...
/-! ### Helper: all steps are stay when cw = ccw = 0 -/
...
/-! ### Helper: fireCount = length when moverAt is constant -/
...
/-! ### Helper: with n ≥ 5, every processor has a "far" processor -/
...
/-! ### Helper: move at a non-mover position is identity -/

private theorem move_ne_eq (c : Config sys.rs) (p q : Fin sys.rs.n) (hq : q ≠ p) :
    (move sys c p) q = c q := by
  simp [move, hq]

/-! ### Helper: no finite cycle in a well-founded relation -/
...
/-! ### Helper: privileged from same context -/
...
/-! ### Helper: config value constant for non-mover -/
...
/-- If a good cycle has zero winding and no clockwise steps, then every step
    is stay, a single processor fires at every step, and flipping a far-away
    processor's value creates a cycle of bad configs contradicting convergence. -/
...
/-- Config value is constant at a processor that never fires. -/
...
/-- If processor q is at distance ≥ 2 from every mover, flipping q creates
    a bad cycle contradicting convergence. -/
...
/-! ### Safe processor implies zero winding -/
...
/-- **Zero-winding large-arc obstruction.**
    Routes to `zeroWinding_consecutive_false` and `zeroWinding_nonConsecutive_false`
    (CaseObstructionsCore) via `large_arc_zeroWinding_ec_proof` (GlobalMinGap). -/
theorem zeroWinding_large_arc_false
  ...
  exact large_arc_zeroWinding_ec_proof hn gc hconv hsub hzero hcw_pos hno_safe
    (fun h3consec => zeroWinding_consecutive_false gc hn hconv hno_safe hsub h3bin hzero hcw_pos h3consec)
    (fun hnoncons => zeroWinding_nonConsecutive_false gc hn hconv hno_safe hsub h3bin hzero hcw_pos hnoncons)

/-- **Non-zero-winding obstruction.**
    Routes sweep and odd-winding non-uniform cases to CaseObstructionsCore stubs. -/
theorem nonZeroWinding_false
    (hn : sys.rs.n ≥ 9) (gc : GoodCycle sys) (hconv : converges sys gc)
    (hno_safe : ¬∃ q : Fin sys.rs.n, ∀ k : Fin gc.configs.length,
      gc.moverAt k ≠ q ∧ gc.moverAt k ≠ left q ∧ gc.moverAt k ≠ right q)
  ...
  · rcases gc.zeroWinding_or_isOddWinding_of_not_sweep hsweep with hzw | hodd
    ...
    · by_cases hunif : gc.uniformDirection
      ...
      · exact oddWinding_nonUniform_false gc hn hconv hno_safe hsub h3bin hodd hunif

/-! ### Main theorem -/

/-- **Sub-threshold obstruction.**

    Any good cycle in a converging sub-threshold system with n ≥ 9 is
    impossible. Routes to CaseObstructionsCore stubs for the 4 case splits.

    Case decomposition:
    1. Safe processor exists → `small_arc_contradicts_convergence'` (sorry-free)
    2. Zero winding, cw = 0 → `all_stay_contradicts_convergence'` (sorry-free)
    3. Zero winding, cw > 0 → `zeroWinding_large_arc_false` →
       A. consecutive binary → `zeroWinding_consecutive_false` (CaseObstructionsCore)
       B. non-consecutive binary → `zeroWinding_nonConsecutive_false` (CaseObstructionsCore)
    4. Non-zero winding → `nonZeroWinding_false` →
       C. sweep → `sweep_false` (CaseObstructionsCore)
       D. odd-winding non-uniform → `oddWinding_nonUniform_false` (CaseObstructionsCore) -/
theorem subThreshold_obstruction_v2
    (hn : sys.rs.n ≥ 9) (gc : GoodCycle sys) (hconv : converges sys gc)
    (hsub : subThreshold sys.rs) :
    False := by
  -- Handle safe-processor case first (sorry-free)
  ...
  · by_cases hzero : gc.zeroWinding
    · by_cases hcw : gc.cwStepCount = 0
      ...
      · exact zeroWinding_large_arc_false hn gc hconv hsub hzero (by omega) hsafe
    · exact nonZeroWinding_false hn gc hconv hsafe hsub hzero
