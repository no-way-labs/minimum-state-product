#!/usr/bin/env python3
"""
Compute the spectrum of pointwise pairs

    (local_fc_delta, correction_gain)

on the explicit-bad non-617 TP-preserving bad boundary-changing class.

Empirically this spectrum is identical for n = 9,10,11,12 and equals:

    { (-2,-2), (-2,-1), (-2,0), (-2,1),
      (-1,-1), (-1,0), (0,-1), (1,-2) }

Together with the global gain laws, this gives the cleanest current finite
inequality target for the positive bridge.
"""

from __future__ import annotations

import sys
from itertools import product
from collections import defaultdict

import cphi_bridge_exact_scan as scan
from cup2_theorem import build_system


def pair_set(n: int):
    ms, fs = build_system(n)
    good = scan.explicit_good_cycle_configs(n)
    bad = [c for c in product(*(range(m) for m in ms)) if c not in good]
    bad_set = set(bad)

    def tp(c):
        return (scan.exp2_count(c, n), scan.int_21(c, n), scan.exp2_weight(c, n))

    fc_cache = {c: scan.fc(c, n) for c in bad}
    tp_cache = {c: tp(c) for c in bad}
    tp_fwd = defaultdict(list)

    for c in bad:
        for i in range(n):
            L, S, R = c[(i - 1) % n], c[i], c[(i + 1) % n]
            out = fs[i](L, S, R)
            if out == S:
                continue
            d = list(c)
            d[i] = out
            d = tuple(d)
            if d in bad_set and tp_cache.get(d) == tp_cache[c]:
                tp_fwd[c].append((d, scan.fc(d, n) - fc_cache[c]))

    g = {c: 0 for c in bad}
    for _ in range(2 * n + 10):
        changed = False
        for c in bad:
            best = g[c]
            for d, dfc in tp_fwd[c]:
                cand = dfc + g[d]
                if cand > best:
                    best = cand
            if best != g[c]:
                g[c] = best
                changed = True
        if not changed:
            break

    phi = {c: fc_cache[c] + g[c] for c in bad}

    out = set()
    for c in bad:
        for i in (0, 1, 2, n - 3, n - 2, n - 1):
            L, S, R = c[(i - 1) % n], c[i], c[(i + 1) % n]
            outv = fs[i](L, S, R)
            if outv == S:
                continue
            d = list(c)
            d[i] = outv
            d = tuple(d)
            if d not in bad_set or tp_cache.get(d) != tp_cache[c]:
                continue
            if scan.encode6(scan.boundary6(c, n)) == scan.encode6(scan.boundary6(d, n)):
                continue
            if (scan.encode6(scan.boundary6(c, n)), scan.encode6(scan.boundary6(d, n))) in scan.EDGE_617:
                continue
            dfc = scan.fc(d, n) - fc_cache[c]
            gain = (phi[d] - scan.fc(d, n)) - (phi[c] - fc_cache[c])
            out.add((dfc, gain))
    return out


def main():
    if len(sys.argv) > 1:
        ns = [int(x) for x in sys.argv[1:]]
    else:
        ns = [9, 10, 11, 12]

    base = None
    base_n = None
    for n in ns:
        p = pair_set(n)
        print(f"n={n}: pair_set={sorted(p)}")
        if base is None:
            base = p
            base_n = n
        else:
            print(f"  same_as_n={base_n}: {p == base}")


if __name__ == "__main__":
    main()
