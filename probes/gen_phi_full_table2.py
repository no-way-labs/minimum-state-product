...
# Φ_full
phi_full = [0] * N
for i in bad_set: phi_full[i] = fc(all_configs[i])
tp_rev = {i: [] for i in bad_set}
for i in bad_set:
    for j, _ in tp_adj[i]: tp_rev[j].append(i)
changed = True
while changed:
    changed = False
    for j in bad_set:
...
# FutureFc
future_fc = [0] * N
for i in bad_set: future_fc[i] = fc(all_configs[i])
    ...
    for j, p in tp_adj[i]:
        ...
        if (future_fc[j] == future_fc[i] and
            ...
            if b1 != b2 and (b1, b2) not in dag_edges:
                print(f"VIOLATION: config {c} --(pos {p})--> {c2}")
                print(f"  boundary: {b1} -> {b2}")
                print(f"  FutureFc: {future_fc[i]} -> {future_fc[j]}")
                print(f"  Φ_full: {phi_full[i]} -> {phi_full[j]}")
                print(f"  fc: {fc(c)} -> {fc(c2)}")
                print(f"  TP: {tp(c)} -> {tp(c2)}")
