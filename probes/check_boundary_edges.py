#!/usr/bin/env python3
"""Check if all B1-B4 anomalous boundary transitions are in the 617-edge sixTupleEdge DAG."""

# sixTupleEdgeVals from SixTuple.lean (617 edges)
sixTupleEdgeVals = [
...
# B1: cN1=0, c0=0, c1=0 → c0 changes to 1
print("=== B1: cN1=0, c0=0, c1=0 → c0: 0→1 ===")
b1_count = 0
b1_in_dag = 0
for c2 in range(3):
    for cN3 in range(3):
        for cN2 in range(3):
            src = encode(0, 0, c2, cN3, cN2, 0)
            tgt = encode(1, 0, c2, cN3, cN2, 0)
            b1_count += 1
            if (src, tgt) in edge_set:
                b1_in_dag += 1
            else:
                print(f"  MISSING: ({src}, {tgt}) = {decode(src)} → {decode(tgt)}")
print(f"  B1: {b1_in_dag}/{b1_count} in DAG")

# B2: cN1=1, c0=1, c1=2 → c0 changes to 0
print("\n=== B2: cN1=1, c0=1, c1=2 → c0: 1→0 ===")
b2_count = 0
b2_in_dag = 0
for c2 in range(3):
    for cN3 in range(3):
        for cN2 in range(3):
            src = encode(1, 2, c2, cN3, cN2, 1)
            tgt = encode(0, 2, c2, cN3, cN2, 1)
            b2_count += 1
            if (src, tgt) in edge_set:
                b2_in_dag += 1
            else:
                print(f"  MISSING: ({src}, {tgt}) = {decode(src)} → {decode(tgt)}")
print(f"  B2: {b2_in_dag}/{b2_count} in DAG")

# B3: cN3=1, cN2=1, cN1=1 → cN2 changes to 2
print("\n=== B3: cN3=1, cN2=1, cN1=1 → cN2: 1→2 ===")
b3_count = 0
b3_in_dag = 0
for c0 in range(2):
    for c1 in range(3):
        for c2 in range(3):
            src = encode(c0, c1, c2, 1, 1, 1)
            tgt = encode(c0, c1, c2, 1, 2, 1)
            b3_count += 1
            if (src, tgt) in edge_set:
                b3_in_dag += 1
            else:
                print(f"  MISSING: ({src}, {tgt}) = {decode(src)} → {decode(tgt)}")
print(f"  B3: {b3_in_dag}/{b3_count} in DAG")

# B4: cN2=2, cN1=0, c0=0 → cN1 changes to 1
print("\n=== B4: cN2=2, cN1=0, c0=0 → cN1: 0→1 ===")
b4_count = 0
b4_in_dag = 0
for c1 in range(3):
    for c2 in range(3):
        for cN3 in range(3):
            ...
            if (src, tgt) in edge_set:
                ...
                print(f"  MISSING: ({src}, {tgt}) = {decode(src)} → {decode(tgt)}")
print(f"  B4: {b4_in_dag}/{b4_count} in DAG")

# Also check: do ALL boundary copy steps produce sixTupleEdge?
# A boundary copy step changes one of the 6 positions, with the output being a copy of L or R
print("\n=== Summary ===")
print(f"Total sixTupleEdgeVals: {len(sixTupleEdgeVals)}")
