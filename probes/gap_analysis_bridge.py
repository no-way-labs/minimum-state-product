#!/usr/bin/env python3
"""Analyze the 135 extra CF+TP edges not in CΦ DAG."""
import sys, os
...
rev = {i: [] for i in bad}
for i in bad:
    for j in all_adj[i]: rev[j].append(i)
changed = True
while changed:
    changed = False
    for j in bad:
...
# Find extra CF+TP 6-tuple edges (not in CΦ DAG)
extra = set()
extra_instances = {}
for i in bad:
    c = bad[i]['c']
    for j in tp_adj[i]:
        ...
        if future_fc[j] == future_fc[i]:
            ...
            if b1 != b2 and (b1, b2) not in dag_edges:
                extra.add((b1, b2))
                if (b1, b2) not in extra_instances:
                    extra_instances[(b1, b2)] = (i, j, phi_full[i], phi_full[j])

print(f"Extra CF+TP 6-tuple edges (not in 617-DAG): {len(extra)}")
print(f"\nAll extra edges with Φ_full change:")
all_phi_drop = True
for (a, b) in sorted(extra):
    i, j, pa, pb = extra_instances[(a, b)]
    dphi = pb - pa
    if dphi >= 0:
        all_phi_drop = False
    print(f"  {a:3d}->{b:3d}  Φ_full: {pa}->{pb} (Δ={dphi:+d})  fc: {bad[i]['fc']}->{bad[j]['fc']}")

if all_phi_drop:
    print(f"\n*** ALL extra edges have Φ_full strictly decreasing ***")
