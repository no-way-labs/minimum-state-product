#!/usr/bin/env python3
from __future__ import annotations

import os
import sys

ROOT = os.path.dirname(os.path.dirname(__file__))
GPT_SCRIPTS = os.path.join(ROOT, "gpt", "scripts")
sys.path.insert(0, GPT_SCRIPTS)

from p2_good_cycle_search import enumerate_good_cycles  # type: ignore


def local_five(t: int, n: int) -> set[int]:
    return {(t - 2) % n, (t - 1) % n, t, (t + 1) % n, (t + 2) % n}


def pivots(ms: tuple[int, ...]) -> list[int]:
    n = len(ms)
    return [i for i in range(n) if ms[(i - 1) % n] == 2 and ms[(i + 1) % n] == 2]


def hk_last_instances(movers: tuple[int, ...], n: int, pivs: list[int]) -> list[tuple[int, int]]:
    hits = []
    for t in pivs:
        outside = [idx for idx, mover in enumerate(movers) if mover not in local_five(t, n)]
        if outside and outside[-1] + 1 == len(movers):
            hits.append((t, outside[-1]))
    return hits


def interval_right(i0: int, t: int, n: int) -> list[int]:
    out = []
    cur = i0
    while True:
        out.append(cur)
        if cur == t:
            return out
        cur = (cur + 1) % n


def interval_left(i0: int, t: int, n: int) -> list[int]:
    out = []
    cur = i0
    while True:
        out.append(cur)
        if cur == t:
            return out
        cur = (cur - 1) % n


def main() -> None:
    n = 7
    ms = (2,) * n
    checked = 0
    failures = 0
    for cycle, movers in enumerate_good_cycles(ms, max_cycles=80, time_limit=8.0):
        hits = hk_last_instances(movers, n, pivots(ms))
        if not hits:
            continue
        i0 = movers[0]
        c0 = cycle[0]
        for t, k_out in hits:
            # choose the shorter interval between i0 and t
            r = interval_right(i0, t, n)
            l = interval_left(i0, t, n)
            path = r if len(r) <= len(l) else l
            first = None
            for j, cfg in enumerate(cycle):
                if cfg[t] != c0[t]:
                    diff = {p for p in path if cfg[p] != c0[p]}
                    first = (j, tuple(path), tuple(sorted(diff)))
                    if set(diff) != set(path):
                        failures += 1
                        print(f"FAIL movers={movers} i0={i0} t={t} first={first} cfg={cfg}")
                    break
            checked += 1
            if checked <= 12 and first is not None and failures == 0:
                print(f"OK movers={movers} i0={i0} t={t} first={first}")
    print(f"\nchecked={checked} failures={failures}")


if __name__ == "__main__":
    main()
