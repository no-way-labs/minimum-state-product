#!/usr/bin/env python3
"""Generate Φ_full lookup table for n=9 to embed in Lean."""
import sys, os
...
# Compute Φ_full via BFS from each config
phi_full = [0] * N  # 0 for good configs
for i in bad_set:
    phi_full[i] = fc(all_configs[i])

# Iterative max-propagation
changed = True
while changed:
    changed = False
    for j in bad_set:
        for i in (tp_adj.get(j) or []):
            # i -> j is a TP-preserving step, so reachable from j includes reachable from...
            # Wait, we need REVERSE: if i->j, then reachable(i) includes reachable(j)
            pass

# Redo: for Φ_full, propagate backward
...
while changed:
    ...
    for j in bad_set:
        for i in tp_rev[j]:
            if phi_full2[j] > phi_full2[i]:
                phi_full2[i] = phi_full2[j]
                changed = True

# Also compute FutureFc
...
for i in bad_set:
    ...
    for j in tp_adj[i]:
        ...
        if (future_fc[j] == future_fc[i] and
            phi_full2[j] == phi_full2[i]):
            b1, b2 = boundary6(c), boundary6(c2)
            if b1 != b2:
                if (b1, b2) not in dag_edges:
                    violations += 1

print(f"Bridge violations at n={n}: {violations}")
print(f"Φ_full range: {min(phi_full2[i] for i in bad_set)}..{max(phi_full2[i] for i in bad_set)}")
print(f"FutureFc range: {min(future_fc[i] for i in bad_set)}..{max(future_fc[i] for i in bad_set)}")

# Output Φ_full table
print(f"\n-- Φ_full lookup table ({N} entries)")
print(f"-- Good configs (fc=0) have Φ_full = 0")
vals = phi_full2
lines = []
for i in range(0, N, 20):
    chunk = vals[i:i+20]
    lines.append("    " + ", ".join(str(v) for v in chunk))
print("def phiFullVals9 : List Nat :=")
print("  [" + ",\n".join(lines) + "]")

# Output FutureFc table too
print(f"\ndef futureFcVals9 : List Nat :=")
vals2 = future_fc
lines2 = []
for i in range(0, N, 20):
    chunk = vals2[i:i+20]
    lines2.append("    " + ", ".join(str(v) for v in chunk))
print("  [" + ",\n".join(lines2) + "]")
